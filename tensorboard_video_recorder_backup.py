...
class TensorboardVideoRecorder(VecEnvWrapper):
    ...
    def _log_video_to_tensorboard(self, tag, video, step):
        print(f"Logging {video.shape[0]} frames to TensorBoard at step {step}")
    ...
    def _record_frame(self):
        ...
        if frame is not None:
            ...
            print(f"Recording frame: {len(self._recorded_frames)}")

    def _finalize_video(self):
        if not self._recorded_frames:
            print("No frames recorded!")
            return
        print(f"Finalizing video with {len(self._recorded_frames)} frames")
